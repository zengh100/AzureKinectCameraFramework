...
{
	public:
	void onDataEvent(float roll, float pitch, cv::Mat mat8UC1, k4a::image depth_image, k4a::image pointCloud, k4a::image color_image) 
	{
		std::cout << "roll = " << roll*rad_to_deg << " deg, pitch = " << pitch*rad_to_deg << " deg\n";
		std::cout << mat8UC1.cols << ", " << mat8UC1.rows << "\n";
		// cv::Mat mat8UC3;
        // cv::cvtColor(mat8UC1, mat8UC3, CV_GRAY2RGB);
...
void TestClass_AzureKinectCamera(std::string video_filename)
{
	...
	camera.setExtractColorImage(true);
	camera.run();
} 
